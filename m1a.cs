...
//------ Part A ------
        ...
        static void DrawArray( )
        {
            ...
            bool isPrime;
            bool nextIsPrime;
            /*when not prime and next/previous not prime set lines to spaces*/
            ...
            for( int r = 0; r < rows; r ++ )
            {
                ...
                for( int c = 0; c < cols; c ++ )
                {
                    isPrime = Prime.IsPrime(a[r,c]);
                    
                    if (c<cols-1) nextIsPrime = Prime.IsPrime(a[r,c+1]); else nextIsPrime = true;
                    if( c == 0 && isPrime) // left edge of first cell
                    {
                        Write( "{0}", v );
                    }
                    if (isPrime) Write( "{0,3} ", a[ r, c ] ); else Write("    "); // the cell content
                    if(isPrime || nextIsPrime) Write( "{0}", v ); else Write(" "); // right edge of cell
                }
                WriteLine( );
                ...
                if( r != rows - 1 )
                {
                    ...
                    for( int c = 0; c < cols; c ++ )
                    {
                        ...
                        if (isPrime) Write( "{0}{0}{0}{0}", h ); else Write("    "); // below the cell content
                        if( c == cols - 1 )  // far right edge
                        {
                            Write( "{0}", vl );
                        }
