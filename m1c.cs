...
//------ Part C ------
        ...
        static void PrimeIslands( )
        {
            ...
            while(!done)
            {
                for(int r=0; r<primeCount-1;r++)
                {
                    for(int c=0; c<primeCount-1; c++)
                    {
                        if (r!=c && 
                            Math.Abs(aPrimeRows[r]-aPrimeRows[c])<=1 && 
                            Math.Abs(aPrimeCols[r]-aPrimeCols[c]))
                        {
                            if(aIslandIds[r] > aIslandIds[c]) aIslandIds[r] = aIslandIds[c];
                            else aIslandIds[r] = aIslandIds[c];
                        }
                    }
